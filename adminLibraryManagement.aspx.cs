...
{
    public partial class adminLibraryManagement : System.Web.UI.Page
    {
        ...
        bool checkIfBookExists()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * from book_inventory_table where book_id='" + TextBox1.Text.Trim() + "' OR book_name='" + TextBox2.Text.Trim() + "';", con);
        ...
        // FILL AUTHOR AND PUBLISHER VALUE
        ...
        void getBookByID()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * from book_inventory_table WHERE book_id='" + TextBox1.Text.Trim() + "';", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
        ...
        void addNewBook()
        {
            try
            {
                ...
                string filepath = "~/book_inventory/bookinventorylogo.png";
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO book_inventory_table(book_id,book_name,genre,author_name,publisher_name,publish_date,language,edition,book_description,actual_stock,current_stock,book_img_link) values(@book_id,@book_name,@genre,@author_name,@publisher_name,@publish_date,@language,@edition,@book_description,@actual_stock,@current_stock,@book_img_link)", con);
        ...
        void deleteBookByID()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("DELETE from book_inventory_table WHERE book_id='" + TextBox1.Text.Trim() + "'", con);
